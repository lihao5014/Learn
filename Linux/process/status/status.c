...
/*5.孤儿进程：
 *（1）一个父进程退出，而它的一个或多个子进程还在运行，那么那些子进程将成为孤儿进程。孤儿进程将被init进程所收养，
 *     并由init进程对它们完成状态收集工作。
 *（2）子进程死亡需要父进程来处理，那么意味着正常的进程应该是子进程先于父进程死亡。当父进程先于子进程死亡时，
 *     子进程死亡时没父进程处理，这个死亡的子进程就是孤儿进程。但孤儿进程与僵尸进程不同的是，由于父进程已经死亡，
 *     系统会帮助父进程回收处理孤儿进程。所以孤儿进程实际上是不占用资源的，因为它终究是被系统回收了。不会像僵尸
 *     进程那样占用ID，损害运行系统。
 */

/*6.僵尸进程：
 *（1）子进程先于父进程退出后，子进程的PCB需要其父进程释放，但是父进程并没有释放子进程的PCB，这样的子进程就称为
 *     僵尸进程。僵尸进程实际上是一个已经死掉的进程。使用ps命令可以查看进程信息后带有<defunct>的都是僵尸进程。
 *（2）一个进程在调用exit命令结束自己的生命的时候，其实它并没有真正的被销毁，而是留下一个称为僵尸进程（Zombie）的
 *     数据结构。在Linux进程的状态中，僵尸进程是非常特殊的一种，它已经放弃了几乎所有内存空间，没有任何可执行代码，
 *     也不能被调度，仅仅在进程列表中保留一个位置，记载该进程的退出状态等信息供其他进程收集，除此之外僵尸进程不再
 *     占有任何内存空间。僵尸进程需要它的父进程来为它收尸，如果他的父进程没有处理这个僵尸进程的措施，那么它就一直
 *     保持僵尸状态。如果这时父进程结束了，那么init进程自动会接手这个子进程，为它收尸，它还是能被清除的。但是如果
 *     父进程是一个循环，不会结束，那么子进程就会一直保持僵尸状态，这就是为什么系统中有时会有很多僵尸进程的原因。
 *（3）如果有大量的僵尸进程驻在系统之中，必然消耗大量的系统资源。但是系统资源是有限的，因此当僵尸进程达到一定数目时，
 *     系统因缺乏资源而导致奔溃。
 */

/*7.僵尸进程的处理方式：
 *（1）一种比较暴力的做法是将其父进程杀死，那么它的子进程，即僵尸进程会变成孤儿进程，由系统来回收。
 *     但是这种做法在大多数情况下都是不可取的，如父进程是一个服务器程序，如果为了回收其子进程的资源，
 *     而杀死服务器程序，那么将导致整个服务器崩溃，得不偿失。显然这种回收进程的方式是不可取的，但其也
 *     有一定的存在意义。
 *（2）使用wait函数来处理僵尸进程。但是进程一旦调用了wait()，就立即阻塞自己，由wait自动分析是否当前进程
 *     的某个子进程已经退出，如果让它找到了这样一个已经变成僵尸的子进程，wait就会收集这个子进程的信息，
 *     并把它彻底销毁后返回；如果没有找到这样一个子进程，wait()就会一直阻塞在这里，直到有一个出现为止。
 *（3）使用SIGCHLD信号来处理僵尸进程。由于调用wait之后就必须阻塞，直到有子进程结束，所以这样来回收僵尸进程是
 *     非常不高效的。实际上当子进程终止时，内核就会向它的父进程发送一个SIGCHLD信号，父进程可以选择忽略该信号，
 *     也可以提供一个接收到信号以后的处理函数。我们不希望有过多的僵尸进程产生，所以当父进程接收到SIGCHLD信号
 *     后就应该调用wait()或waitpid()函数对子进程进行善后处理，释放子进程占用的资源。即子进程什么时候执行完毕，
 *     就什么时候通知父进程去，回收子进程资源。这样也就避免了父进程一直阻塞等待回收子进程的问题。
 */
