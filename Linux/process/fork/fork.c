 ...
 *     理函数接收处理就可以避免僵尸进程。
 *（4）fork就是基于写时复制，只读代码段是可以共享的。
 */

/*若使用vfork则子进程和父进程占用同一个内存映像，在子进程修改会影响父进程。同时只有在子进程执行
 *exec/exit之后才会运行父进程。实际上子进程占用的栈空间就是父进程的栈空间，所以需要非常小心。
 *如果vfork的子进程并没有exec或者是exit的话，那么子进程就会执行直到程序退出之后，父进程才开始执行。
 *而这个时候父进程的内存已经完全被写坏。
 */
...
/*父子进程的代码从fork()函数的返回开始，分别在两个地址空间中同时运行，从而使两个进程分别获得其所属
 ...
 *     fork()：子进程共享父进程的地址空间。准确来说在调用exec进程替换或exit退出进程之前与父进程数据是共享的。
 */
