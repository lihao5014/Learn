...
/*普通函数在调用时，只需要保证调用位置之前有相应的声明即可；而常量表达式函数则不同，
 *调用位置之前必须要有该函数的定义，否则会导致程序编译失败。
 */
...
void fun(const int num)
{
	...
	/*const修饰了变量b，表示该变量“只读”，即无法通过变量自身去修改自己的值。但这并不意味着变量b的值
	 *不能借助其它变量间接改变，通过改变a的值就可以使b的值发生变化。
	 */
	int a = 20;
	const int &b = a;  //变量只读并不等价于常量，二者是两个概念不能混为一谈。“只读”和“不允许被修改”之间并没有必然的联系。
 ...
 *当定义struct或class时，添加constexpr关键字修饰是无效的。
 */
