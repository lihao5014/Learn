 ...
 *     只有extern关键字，而没有extern "C"关键字。extern "C"是C++语言的关键字，C语言编译器并不认识。
 */
 
/*2.C与C++语言的编译方式：
 *（1）C++能够兼容C语言的编译方式，但是C++编译器会默认以C++的方式来编译C代码。而C编译器却不能编译C++代码，
 *     只能通过动态库的方式来调用C++实现的功能。C语言能以源码的方式供C++程序调用，但是C++却只能以库的方式
 *     供C语言调用。动态库是实现不同程序设计语言间相互调用的通用方式。
 *（2）一般.cpp文件是采用g++去编译，而.c文件是采用gcc去编译，但是这不是绝对的。gcc和g++都可以编译.c文件，
 *     也都可以编译.cpp文件。g++和gcc是通过文件后缀名来辨别是c程序还是c++程序的。这一点与Linux辨别文件的
 *     方式不同，Linux是通过文件信息头辨别文件的。
 *（3）在gcc看来.c文件会以C方式去编译，而.cpp文件则是以C++的方式去编译。gcc不会主动去链接C++用到的stdc++库，
 *     所以用gcc编译cpp文件时需要手动指定链接选项-lstdc++。而对于g++不管是.c还是.cpp文件都是以C++方式去编译。
 *（4）__cpluscplus并不是g++编译器中才会定义的宏，而是任何以C++的方式去编译都会定义的宏。即当gcc编译.cpp文件、
 *     g++编译.c/.cpp文件，__cplusplus宏都会被编译器定义。
 ...
 *（6）对于一份.c文件，采用gcc编译时候没有定义__cplusplus，#ifdef宏判断不起作用，自身以c语言的方式编译。
 *     采用g++编译时定义了_cplusplus，经过#ifdef宏判断后，还是会以c语言的方式编译。注意extern “C”是g++才
 *     具有的关键字，gcc并没有，所以如果用gcc编译而不加以宏判断直接使用extern “C”那么就会出现语法错误。
 */
