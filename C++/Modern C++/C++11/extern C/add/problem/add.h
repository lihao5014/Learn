 ...
 *    0000000000000000 t .text
 *    0000000000000000 r .xdata
 *    0000000000000000 T _Z3addi       //因为C++存在函数重载，所以在编译时会将函数参数类型和个数一并加入符号中。
 *    0000000000000000 ? add.c
 *
 *（1）使用gcc和g++去编译同一个.c文件生成二进制对象文件中，add()函数的符号名不相同。C方式编译和C++方式编译，
 *     其差异就在于符号表标识符。同一个函数名，在C方式编译的其函数名跟编译前的函数一致，C++方式编译的则是以
 *     函数名结合参数作为编译后的函数名。要确保文件以.c方式编译，可以利用__cplusplus这个宏在C++编译的方式才
 *     会定义的宏，再结合extern "C"用法。
 *（2）C++代码中直接去调用C语言制作的libadd_gcc.a库，会报“未定义的add()函数”错误。这是因为main.cpp是C++文件，
 *     用gcc或g++编译时，会优先选择cpp的编译方式，即会用cpp的编译方式去引用add()函数。然而libadd_gcc.a库又是
 *     用c语言的方式去编译的。由于cpp编译器是兼容C语言的编译方式的，所以在编译cpp文件调用到.c文件中函数的地方，
 *     需要用extern "C"指定用c语言的方式去编译它。
 */
