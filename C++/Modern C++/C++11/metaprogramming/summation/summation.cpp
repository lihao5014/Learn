 ...
 *（3）C++11新增了一些模版元编程相关的特性，不仅可以让我们编写模版元程序变得更容易，还进一步增强
 *     了泛型编程的能力，比如type_traits让我们不必再重复发明轮子了，给我们提供了大量便利的元函数，
 *     还提供了可变模板参数和tuple，让模版元编程“如虎添翼”。
 */
 ...
 *     可用的语法元素相当有限。
 *（5）实际上模板元中的if-else可以通过type_traits来实现，它不仅仅可以在编译期做判断，还可以做计算、查询、
 *     转换和选择。
 *（6）模板元中的for等逻辑可以通过递归、重载和模板特化（偏特化）等方法实现。
 */
 ...
 *（5）特性traits：又称为特性萃取技术，说得简单点就是提取“被传进的对象”对应的返回类型，让同一个接口实现
 *     对应的功能。因为STL的算法和容器是分离的，两者通过迭代器链接。算法的实现并不知道自己被传进来什么。
 *     萃取器相当于在接口和实现之间加一层封装，来隐藏一些细节并协助调用合适的方法，这需要偏特化等技巧。
 *（6）策略policy：一般是一个类模板，典型的策略是STL容器的分配器（这个参数有默认参数，即默认存储策略），
 *     策略类将模板的经常变化的那一部分子功能块集中起来作为模板参数，这样模板便可以更为通用，这和特性
 *     的思想是类似的。
 *（7）标签tag：一般是一个空类，其作用是作为一个独一无二的类型名字，用于标记一些东西。典型的例子是STL
...
/*6.C++模板元编程的优缺点：
 *（1）C++模板元编程是图灵完备的且是函数式编程，主要特点是代码在编译期执行，可用于编译期数值计算，
 *     能够获得更有效率的运行码。模板的使用也提高了代码泛化。与此同时C++模板元编程也存一定的缺点。
 *     所以对于模板元编程，我们需要扬其长避其短，合理使用模板元编程。
 *（3）模板元编程产生的代码较为复杂，难易阅读，可读性较差；
 *（4）大量模板的使用，编译时容易导致代码膨胀，提高了编译时间；
 *（5）对于C++来说，由于各编译器的差异，大量依赖模板元编程（特别是最新形式的）的代码可能会有移植性的问题。
 */
