 ...
 *     开发中降低代码冗余也有很好的借鉴意义。
 *（5）traits技术简单的说就是：加上一层间接性，换取一定的灵活性。
 */
...
/*因为模板参数Shape的类型不同，会影响到drawByObject()/drawByType()函数模板中具体的算法实现，
 *那么我们就需要把这些与模板参数相差的方法，从drawByObject()/drawByType()函数模板中提取出来，
 *从而保证drawByObject()/drawByType()算法的一致，不受参数类型不同的影响。增加的一层特性萃取类
 *Painter即是把各类型模板参数之间的差异化方法都封装起来了。
 */
