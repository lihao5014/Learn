/*1.函数式编程的特点:
 *（1）函数式编程期望函数的行为项数学上的函数，而非一个计算机上的“子程序”。这样的函数一般被称为纯函数，
 *     主要体现在确定性。所谓确定性就是像数学中那样，y = f(x)这个函数无论什么场景都会得到同样的结果。
 *     而不是像程序中的很多函数那样。同一个参数在不同的场景下会计算出相同的结果，称之为函数的确定性。
 *     所谓不同的场景，就是我们的函数会根据运行中的状态信息的不同而发生变化。
 *（2）会影响函数结果的只是函数的参数，没有对环境的依赖。
 *（3）函数返回的结果就是函数执行的唯一结果，不产生对环境的其他影响。
 *（4）函数的执行没有顺序上的问题。
 *（5）函数可以像普通的对象一样被传递、使用或返回。
 *（6）函数式编程代码更像是说明式而非命令式。熟悉函数式编程后，你会发现说明式代码的可读性比命令式更高，
 *     代码更短，可复用性更高。
 *（7）函数无状态，没有状态就没有伤害，就像没有依赖就没有伤害一样。
 */
