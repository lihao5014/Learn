 ...
 *（2）pipeline：就是把函数实例成一个一个的action，然后把一组action放到一个数组或是列表中，然后把
 *     数据传给这个action list，数据就像一个pipeline一样顺序地被各个函数所操作，最终得到想要的结果。
 *     pipeline借鉴于Unix Shell的管道操作，把若干个命令串起来，前面命令的输出成为后面命令的输入，
 *     如此完成一个流式计算。
