/*1.编译期常量:
 *编译期常量是C++中相当重要的一部分，整体而言它们有助提高程序的正确性，并提高程序的性能。
 *总有些东西是编译器要求编译期间就要确定的，除了变量的类型外，最频繁出现的地方就是enum枚举、
 *switch的case标签、模板的值类型参数、定义数组的大小和static_assert静态断言。
 */
 ...
 *数组的内存就会被预留在该函数的栈帧中；如果这个数组是类的一个成员，那么编译器
 *要确定数组的大小以确定这个类成员的大小。无论哪种情况，编译器都要知道这个数组
 *具体的size。
 */

/*3.模板中的编译期常量：
 *除了类型以外，数字也可以作为模板的参数。这些数值变量包括int、long、short、bool、char
 *和弱枚举enum等。既然编译器在初始化模板的时候必须知道模板的类型，那么这些模板的参数也
 *必须是编译期常量。
 */
