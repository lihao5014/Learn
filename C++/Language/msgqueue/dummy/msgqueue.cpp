...
void inputMsg()
{
	...
	int opt = -1;
	double x = -EPS,y = -EPS;
	Signal_t signal = {-EPS,-EPS,Action_t(-1)};
	do{
		if(isFull())
		{
		...
		signal.x = x;
		signal.y = y;
		signal.action = static_cast<Action_t>(opt);
		pushMsg(signal);
...
void clearMsg()
{
	for(int i=0;i<SIZE;++i)
	{
		msgQueue[i].x = -EPS;
		msgQueue[i].y = -EPS;
		(msgQueue + i)->action = static_cast<Action_t>(-1);
	}
}
...
bool isEmpty()
{
	for(int i=0;i<SIZE;++i)
	{
		if(msgQueue[i].action != -1)     //C++中赋值需要将-1强转为枚举型，比较不需要强转
		{
			return false;
		}
	}
...
bool isFull()
{
	for(int i=0;i<SIZE;++i)
	{
		if((*(msgQueue + i)).action == (Action_t)-1)
		{
			return false;
		}
	}
...
Signal_t popMsg()
{
	...
	Signal_t ret = {-EPS,-EPS,static_cast<Action_t>(-1)};
	for(int i=0;i<SIZE-1;++i)
	{
		if(msgQueue[i + 1].action == Action_t(-1))
		{
			ret = msgQueue[i];
			msgQueue[i].x = -EPS;
			(*(msgQueue +i)).y = -EPS;
...
int countMsg()
{
	for(int i=0;i<SIZE-1;++i)
	{
		if(msgQueue[i].action == static_cast<Action_t>(-1))
		{
			return i;
		}
	}
